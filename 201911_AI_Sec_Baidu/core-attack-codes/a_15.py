...
add_arg('input',            str,   "./input3_image/",     "Input directory with images")
add_arg('output',           str,   "/home/aistudio/work/attack_example/attack_code/output_image/",    "Output directory with images")
...
pretrained_model="/home/aistudio/work/attack_example/attack_code/models_parameters/ResNeXt50_32x4d"
...
def inference_list(img):
    ...
    result = result[0][0]
    # pred_label = np.argmax(result)
    pred_label = find_sub_max(result.copy(),3)
    # print(pred_label)
    pred_score = []
    for i in pred_label:
        # print(i)
        pred_score.append(result[i].copy())
    return pred_label, pred_score
...
def attack_nontarget_by_FGSM(img, src_label):
    pred_label = src_label[0].copy()

    step = 4.0/64.0
    eps = 32.0/64.0

    while pred_label in src_label:
...
def gen_adv():
    ...
    for filename, label in original_files:
        ...
        prelabel, xxxx = inference_list(img)
        # print(prelabel, xxxx)
        if label in prelabel:
            adv_img = attack_nontarget_by_FGSM(img, prelabel)
        else:
